...
class DailyCtcController extends Controller
{
    ...
    public function calculate_ctc($date)
    {
        ...
        $allEmployees = Employee::where('status', UserStatus::$ACTIVE)->without('branch', 'department', 'designation', 'costcenter', 'subdepartment')->orderBy('finger_id', 'ASC')->pluck('finger_id')->toArray();
        ...
        foreach ($allEmployees as $key => $finger_id) {
            ...
            if (isset($employees[$finger_id])) {
                ...
                if (in_array($singleEmployeeInfo->finger_id, array_values($permenantEmployees))) {
                    ...
                    $CTC = $this->permenantEmployeeCtcDetail($date, $singleEmployeeInfo->finger_id, $permenantEmployeesWithCTC[$singleEmployeeInfo->finger_id]);
                    $permenantEmployeeCTC += $CTC['ctc'];
                ...
                } else if (!in_array($singleEmployeeInfo->finger_id, array_values($permenantEmployees))) {
        ...
        try {
            ...
            $missed > 0 ? info('Some Employee doesnt satisy conditions, there are ' . $missed . ' improper datas') : '';
    ...
    public function permenantEmployeeCtcDetail($date, $finger_id, $ctc)
    {
        ...
        foreach ($employeeInOutData as $key => $inOutData) {
            ...
            if ($inOutData->in_time != null || $inOutData->out_time != null) {
                (float) $dayValue += 1.00;
                $status = 1;
